...
`html-presentation-slides` skill without making them the default for every deck.
...
## How To Use In HTML Presentation Slides
...
If the wildcard is custom, it must follow HTML Presentation Slides' no-slop aesthetics:
...
- Keep `html-presentation-slides` output as one self-contained HTML file.
